...
# reading in the .csv
DataSet <- read.csv(file.choose(), header=TRUE, sep=" ") # **your 'sep' may be different depending on your .csv**
...
MAPshp <- readOGR(dsn=".", layer="Manhattan") # "layer" will be the name of your shapefile, i.e. whatever precedes the file extension '.shp'
MAPraw <- readshpnw(MAPshp, ELComputed=FALSE) # ELComputed and other parameters may change; see documentation for shp2graph package
...
attCatch <- function(tS, tE) { # tripStartNodeIDs and tripEndNodeIDs are passed in
	...
	c(spdlims, strlens) # returns all speed limit groupings and street length groupings
